...
export const MobileLayout: React.FC<MobileLayoutProps> = ({
  ...
  useEffect(() => {
    if (timelineRef.current) {
      const activeBtn = timelineRef.current.children[currentFrameIndex] as HTMLElement;
      if (activeBtn) {
      ...
      <div
        className={
          orientation === "portrait"
            ? "absolute inset-0 z-[999] bg-slate-900 flex flex-col items-center justify-center p-8"
            : "hidden"
        }
      >
        <svg
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Rotate to View</h2>
        <p className="text-slate-400 text-lg text-center">
          Tactical Player works best in landscape mode.
        </p>
      </div>
        ...
        {showMenu && (
          <div className="absolute top-16 left-4 z-20 flex flex-col gap-2 animate-slide-in-left pointer-events-auto">
             <div className="bg-white rounded-xl shadow-xl p-4 w-64 space-y-4">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Load Tactics</h3>
        ...
        <div className="pointer-events-auto bg-white/95 backdrop-blur-md border-t border-slate-200 pb-safe px-4 py-3 flex items-center gap-4">
            ...
            <button
              ...
              className={`flex-none w-12 h-12 flex items-center justify-center rounded-full shadow-lg transition-transform active:scale-95 ${
                isPlaying ? "bg-amber-100 text-amber-600" : "bg-primary-600 text-white"
              }`}
            >
              {isPlaying ? (
            ...
            {/* Timeline Slider */}
            <div className="flex-1 relative h-12 flex items-center" ref={timelineRef}>
